#!/usr/bin/env bash
# Full Fashion-MNIST pipeline using 2 GPUs in parallel (cuda:0 + cuda:1)
set -euo pipefail
...
log "=== CS-WAE Fashion-MNIST Pipeline — 2 GPU parallel ==="
log "Dataset=$DATASET  GPU0=$GPU0  GPU1=$GPU1  seeds=${SEEDS[*]}"

# ── Phase 1: Multi-seed training ─────────────────────────────────────────────
run_bg "seed_0" python train_cs_wae.py \
  --dataset "$DATASET" --seed 0 --device "$GPU0" --output-dir "$RUNS_DIR/seed_0"
P1A=$LAST_BG_PID
run_bg "seed_1" python train_cs_wae.py \
  --dataset "$DATASET" --seed 1 --device "$GPU1" --output-dir "$RUNS_DIR/seed_1" --skip-advanced-viz
P1B=$LAST_BG_PID
wait "$P1A" "$P1B"
log "Done seeds 0 & 1"
...
python train_cs_wae.py \
  --dataset "$DATASET" --seed 2 --device "$GPU0" --output-dir "$RUNS_DIR/seed_2" --skip-advanced-viz \
