...
main_tex="$script_dir/sca_v25_iclr2027.tex"
build_log="$script_dir/sca_v25_iclr2027.log"
pdf_file="$script_dir/sca_v25_iclr2027.pdf"
...
plot_python="$repo_dir/.venv/bin/python"
if [[ ! -x "$plot_python" ]]; then
  echo "ERROR: project Python environment is missing: $plot_python" >&2
  exit 1
fi

"$plot_python" "$repo_dir/scripts/plot_v25_fact_tradeoff.py"
python3 "$repo_dir/scripts/build_v25_evidence_manifest.py"

(
  cd "$script_dir"
  latexmk -pdf -interaction=nonstopmode -halt-on-error sca_v25_iclr2027.tex
)
...
python3 "$repo_dir/scripts/audit_v25_anonymity.py"
