...
\midrule
VAE              & \todo{run} & \todo{run} & \todo{run} & N/A \\
WAE-MMD          & \todo{run} & \todo{run} & \todo{run} & N/A \\
$\beta$-TCVAE    & \todo{run} & \todo{run} & \todo{run} & N/A \\
FactorVAE        & \todo{run} & \todo{run} & \todo{run} & N/A \\
\ours{} (ours)   & $0.0017$ & $6.25$ & \todo{run LP} & $0.15$ \\
\midrule
\ours{} + per-class MMD & \todo{run} & \todo{run} & $\approx 0.10$ & $1.00$ \\
\bottomrule
\end{tabular}
\end{table}
...
zero, and a linear probe trained on $\zs$ achieves \todo{run LP script} accuracy
(chance: $10\%$; run \texttt{scripts/compute\_leakage\_diagnostics.py} on the MNIST
checkpoint to obtain this number).
