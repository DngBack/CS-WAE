...
for label-guided generative clustering---per-class style MMD raises class-conditional
generation self-accuracy from $15\%$ to $100\%$ on MNIST while preserving
$80.87\pm0.52\%$ clustering accuracy on CIFAR-10.
...
of the phenomenon.
With per-class style MMD, self-accuracy reaches $100\%$ while CIFAR-10 clustering
accuracy remains $80.87\pm0.52\%$.
...
\begin{itemize}
  ...
  \item We propose \emph{per-class style MMD}, a general supervised regularizer that
        directly enforces class-conditional style independence, and show it eliminates
        the failure while preserving clustering and reconstruction quality
...
\caption{Conditional style leakage across five model families on MNIST.
         ...
         \todo{Fill in all entries after running cross-model experiments.}}
...
\midrule
VAE              & \todo{} & \todo{} & \todo{} & \todo{} \\
WAE-MMD          & \todo{} & \todo{} & \todo{} & \todo{} \\
$\beta$-TCVAE    & \todo{} & \todo{} & \todo{} & \todo{} \\
FactorVAE        & \todo{} & \todo{} & \todo{} & \todo{} \\
\ours{} (ours)   & $0.0017$ & $6.25$ & \todo{} & $0.15$ \\
\midrule
\ours{} + per-class MMD & $\approx 0$ & $\approx 0$ & $\approx 0.10$ & $1.00$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:cross-model} summarizes the diagnostic measurements.
\todo{Complete the table and add a short paragraph per model discussing why
leakage severity varies.}
...
zero, and a linear probe trained on $\zs$ achieves \todo{\%} accuracy
(chance: $10\%$).
...
\caption{CIFAR-10 ablations. Seed 0.
         ``Leak'' = $\Delta_{\mathrm{inter}}$ on MNIST.
...
\midrule
\ours{} (full, seed 0)                    & 0.8149 & 0.6622 & 0.6438 & \todo{} \\
\ours{} + per-class style MMD             & \todo{} & \todo{} & \todo{} & $\approx 0$ \\
\midrule
\emph{Remove components:}                 & & & & \\
\quad No $\zs$ (semantic-only)            & \todo{} & \todo{} & \todo{} & N/A \\
\quad No class MMD ($\alpha{=}0$)         & \todo{} & \todo{} & \todo{} & \todo{} \\
\quad No per-class style MMD ($\delta{=}0$)& \todo{} & \todo{} & \todo{} & large \\
...
but should drive $\Delta_{\mathrm{inter}}\to 0$ and enable naive prior sampling
with $\zs\sim\mathcal{N}(0,I)$ to achieve self-accuracy $\approx 100\%$.
We compare FID under global Gaussian sampling before and after adding per-class
style MMD to quantify the generation improvement.
\todo{Fill in FID comparison after training with per-class style MMD.}
...
\emph{Per-class style MMD} is a training-time regularizer that enforces
class-conditional style independence directly.
The \emph{class-conditional style prior} is a post-hoc correction that restores
generation correctness without retraining.

Instantiated in F-CS-WAE---a factorized Spherical Cauchy Wasserstein auto-encoder
for label-guided generative clustering---these remedies raise MNIST class-conditional
generation self-accuracy from $15\%$ to $100\%$ while maintaining
$80.87\pm0.52\%$ clustering accuracy on CIFAR-10.
...
claiming reliable class-conditional generation.
...
\begin{itemize}
  \item Complete cross-model experiment (Table~\ref{tab:cross-model}): run VAE,
        WAE-MMD, $\beta$-TCVAE, FactorVAE on MNIST with leakage diagnostics.
  \item Add Figure: bar plot of global MMD vs.\ $\Delta_{\mathrm{inter}}$ across
        model families.
  \item Add t-SNE plots of $\zs$ colored by class label for each model family.
  \item Run \ours{} on MNIST and Fashion-MNIST for seeds 0, 1, 2 and report
        mean$\pm$std.
  \item Tune $\delta_{\mathrm{final}}$ for per-class style MMD; run 3 seeds on
        CIFAR-10 and MNIST; fill ablation Table~\ref{tab:ablations}.
  \item Recompute FID with global Gaussian sampling after training with per-class
        style MMD; compare against FID with class-conditional style prior.
  \item Add LP accuracy ($\mathrm{LP}(\zs\to y)$) and HSIC$(\zs,y)$ to all main
        and ablation tables.
  \item Run all baselines for seeds 1 and 2; update Table~\ref{tab:cifar-baselines}.
