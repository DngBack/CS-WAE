#!/usr/bin/env bash
# Resume Fashion-MNIST pipeline after seed_0/seed_1 (skip completed steps)
set -euo pipefail
...
PIPELINE_LOG="$LOG_DIR/pipeline_${DATASET}_resume_${TIMESTAMP}.log"
...
log "=== Resume Fashion-MNIST pipeline ==="
...
if [[ ! -f "$RUNS_DIR/seed_2/metrics.json" ]]; then
  ...
  python train_cs_wae.py \
    ...
    --output-dir "$RUNS_DIR/seed_2" --skip-advanced-viz \
