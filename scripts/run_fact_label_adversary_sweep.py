#!/usr/bin/env python3
"""Run the frozen MNIST nonlinear style-label adversary development sweep."""
...
RUN_ROOT = ROOT / "runs_fact" / "label_adversary_mnist_40e"
AUDIT_ROOT = ROOT / "runs_diag" / "fact" / "label_adversary_mnist_40e"
LOG_ROOT = ROOT / "logs" / "fact_label_adversary_mnist_40e"
...
REFERENCES = {
    ...
    "label_hsic_w100": ROOT / "runs_fact" / "label_hsic_mnist_40e"
    / "label_hsic_w100" / "seed_0" / "f_cs_wae_model.pth",
}
RUNS = [
    {"name": "label_adv_w003", "weight": 0.03},
    {"name": "label_adv_w01", "weight": 0.10},
    {"name": "label_adv_w03", "weight": 0.30},
]
COMMON = [
...
def checkpoint(run: dict) -> Path:
    return run_directory(run) / "f_cs_wae_model.pth"


def complete(run: dict) -> bool:
    history = run_directory(run) / "training_history.json"
    ...
    for run in runs:
        if not complete(run):
            command = [
                ...
                "--fact-label-adversary-weight", str(run["weight"]),
                "--device", f"cuda:{gpu}",
                "--output-dir", str(run_directory(run)),
            ]
...
def audit_one(name: str, path: Path, gpu: int) -> None:
    command = [
        ...
        "--seed", "0", "--out",
        str(AUDIT_ROOT / f"leakage_{name}_seed0.json"),
    ]
...
def terminal_training_metrics(name: str) -> dict:
    if not name.startswith("label_adv_"):
        return {}
    history = json.loads(
        (RUN_ROOT / name / "seed_0" / "training_history.json").read_text()
    )
    final = history[-1]
    return {
        "train_adversary_accuracy": final["label_adversary_accuracy"],
        "train_adversary_ce": final["label_adversary_ce"],
        "train_adversary_confusion": final["label_adversary_confusion"],
    }
...
def summary_row(name: str, path: Path, joint: dict) -> dict:
    ...
    row.update(terminal_training_metrics(name))
    return row
...
def build_summary() -> dict:
    ...
    rows = [
        summary_row(name, path, joint)
        for name, path in all_checkpoints().items()
    ]
    control = next(row for row in rows if row["name"] == "cap_control")
    hsic_base = next(row for row in rows if row["name"] == "label_hsic_w100")
    for row in rows:
        ...
        row["mlp_reduction_vs_hsic100"] = (
            hsic_base["sample_mlp_accuracy"] - row["sample_mlp_accuracy"]
        )
        gates = {
    ...
    candidates = sorted(
        [row for row in rows if row["name"].startswith("label_adv_")],
        key=lambda row: (
            -row["n_gates_passed"], row["sample_mlp_accuracy"],
            row["conditional_hsic"], row["joint_mmd"],
        ),
    )
    best = candidates[0]
    promotion = (
    ...
    return {
        "protocol": "fact-style-label-adversary-mnist-40e-v1",
        "created_at_utc": now(), "model_seed": 0, "evaluation_seed": 0,
        "recommended_candidate": best["name"],
        "promotion_allowed": promotion, "rows": rows,
    }
...
def main() -> None:
    ...
    try:
        ...
        assignments = [
            items[index::len(args.gpus)] for index in range(len(args.gpus))
        ]
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(args.gpus)) as pool:
            futures = [
        ...
        atomic_json(AUDIT_ROOT / "label_adversary_summary.json", summary)
        manifest.update(
