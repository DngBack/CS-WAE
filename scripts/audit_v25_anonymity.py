#!/usr/bin/env python3
"""Audit the v25 submission-facing snapshot for common identity leaks.
...
MANIFEST = ROOT / "fcswae" / "v25_evidence_manifest.json"
OUTPUT = ROOT / "fcswae" / "v25_anonymity_report.json"
SIDECAR = OUTPUT.with_suffix(OUTPUT.suffix + ".sha256")
PDF = ROOT / "fcswae" / "sca_v25_iclr2027.pdf"
...
BASE_TARGETS = [
    ...
    "fcswae/v25_evidence_manifest.json",
]
...
def manuscript_graphics() -> list[str]:
    ...
    for manuscript in (
        ROOT / "fcswae" / "sca_v25_iclr2027.tex",
        ROOT / "fcswae" / "appendix_v25_iclr2027.tex",
    ):
...
def main() -> None:
    if not MANIFEST.is_file():
        raise FileNotFoundError(
            "generate fcswae/v25_evidence_manifest.json before the anonymity audit"
        )
    target_names = sorted(
    ...
    payload = {
        "schema": "iclr2027-v25-anonymity-audit-1.0.0",
        ...
        "boundary": (
            "The audit covers v25 manuscript sources, included figures, the "
            "submission README/figure-source index, and every source/artifact "
            "listed in the v25 evidence manifest. It does not certify files that "
